...
void print_usage(const char* program_name) {
    std::cout << "═══════════════════════════════════════════════════════\n";
    std::cout << "  G07 v10.0 - Kolmogorov Lossless Compression\n";
    std::cout << "═══════════════════════════════════════════════════════\n";
...
int main(int argc, char* argv[]) {
    // G07 v10.0 - Kolmogorov compression + v9 pipeline
    if (argc != 3) {
        print_usage(argv[0]);
        return 1;
    }
    ...
    try {
        ...
        const bool candidate_logging_enabled = env_flag_enabled("G07_V10_CANDIDATE_LOG");
        ...
        if (auto_select) {
            ...
            if (detected_entropy > 7.5) {
                // High entropy — might be PRNG output (Kolmogorov compressible)
                std::cout << "Entropy " << detected_entropy << " > 7.5 — testing PRNG hypothesis..." << std::endl;
                auto prng_result = PrngModel::detect(input_data, 65535, 64);
                if (prng_result.detected) {
                    model_type = ModelType::PRNG_DETECTED;
                    std::cout << "Decision: PRNG DETECTED! Algorithm=glibc_rand, Seed="
                              << prng_result.seed << std::endl;
                    std::cout << "  Shannon entropy: " << detected_entropy
                              << " bits/byte (appears random)" << std::endl;
                    std::cout << "  Kolmogorov complexity: ~14 bytes (PRNG seed + algo + length)" << std::endl;

                    // Write PRNG compressed format:
                    // [0x0D: ModelType][Original Size: 8B][Algorithm: 1B][Seed: 4B]
                    uint64_t prng_orig_size = input_data.size();
                    std::vector<uint8_t> output_data;
                    output_data.push_back(static_cast<uint8_t>(ModelType::PRNG_DETECTED));
                    for (int i = 0; i < 8; i++)
                        output_data.push_back((prng_orig_size >> (i * 8)) & 0xFF);
                    output_data.push_back(static_cast<uint8_t>(prng_result.algorithm));
                    for (int i = 0; i < 4; i++)
                        output_data.push_back((prng_result.seed >> (i * 8)) & 0xFF);
                    ...
                    std::cout << "Model: PRNG Detected (Kolmogorov compression)" << std::endl;
                    std::cout << "  Algorithm: glibc rand() (TYPE_3 additive feedback)" << std::endl;
                    std::cout << "  Seed: " << prng_result.seed << std::endl;
                ...
                std::cout << "Decision: UNCOMPRESSED (no PRNG pattern found, entropy incompressible)" << std::endl;
