...
int main(int argc, char* argv[]) {
    if (argc != 3) {
        std::cout << "═══════════════════════════════════════════════════════\n";
        std::cout << "  G07 v10.0 - Kolmogorov Lossless Decompression\n";
        std::cout << "═══════════════════════════════════════════════════════\n";
        std::cout << "\nUsage: " << argv[0] << " <compressed_file> <output_file>\n\n";
        std::cout << "Handles v10 formats (model types 0x00-0x08, 0x0D, 0xFF)\n";
        std::cout << "  • PRNG Detected (0x0D) - Kolmogorov decompression\n";
    ...
    try {
        ...
        if (!compressed_data.empty()) {
            ...
            if (model_type_byte > 0x0A && model_type_byte != 0x0D && model_type_byte != 0xFF) {
        ...
        // Handle PRNG detected format (Kolmogorov decompression)
        if (!compressed_data.empty() &&
            static_cast<ModelType>(compressed_data[0]) == ModelType::PRNG_DETECTED) {

            // Format: [0x0D][Original Size: 8B][Algorithm: 1B][Seed: 4B] = 14 bytes
            if (compressed_data.size() < 14) {
                throw std::runtime_error("Invalid PRNG compressed file: too small");
            }
            ...
            auto algorithm = static_cast<PrngModel::Algorithm>(compressed_data[off++]);

            uint32_t seed = 0;
            for (int i = 0; i < 4; i++)
                seed |= static_cast<uint32_t>(compressed_data[off++]) << (i * 8);

            std::cout << "Model type: PRNG Detected (Kolmogorov decompression)" << std::endl;
            std::cout << "  Algorithm: glibc rand() (TYPE_3 additive feedback)" << std::endl;
            std::cout << "  Seed: " << seed << std::endl;
            std::cout << "Original size: " << original_size << " bytes" << std::endl;
            std::cout << "Regenerating data from PRNG seed..." << std::endl;

            std::vector<uint8_t> output_data = PrngModel::generate(algorithm, seed, original_size);
